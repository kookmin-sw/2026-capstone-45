"""passage semantic artifact 기반의 원본 문서 검색 도구.

검색 단위는 OCR block이 아니라 SemanticArtifact의 content passage다.
heading, body, table, caption/footnote가 함께 묶인 passage를 검색 record로
인덱싱하고, excluded block은 검색 index에 넣지 않는다.
"""
...
COLLECTION_NAME = "docs_v3_passage_semantic"
...
def _collection_name_for_sources(
    source_doc_infos: Sequence[dict[str, Any]] | None,
    semantic_artifacts: Sequence[SemanticArtifact | None] | None = None,
) -> str:
    if not source_doc_infos and not semantic_artifacts:
        return COLLECTION_NAME
    source_identity = [
        {
            "tool_document_id": info.get("tool_document_id"),
            "doc_id": info.get("doc_id"),
            "display_name": info.get("display_name"),
        }
        for info in source_doc_infos or []
    ]
    semantic_identity = [
        {
        ...
        for artifact in semantic_artifacts or []
        if artifact is not None
    ]
    identity = {"sources": source_identity, "semantic": semantic_identity}
    digest = hashlib.sha1(json.dumps(identity, sort_keys=True, ensure_ascii=False).encode("utf-8")).hexdigest()[:12]
    ...
    for record in records:
        lexical_text = "\n".join(
            [
                ...
                record.passage_title or "",
                record.passage_summary or "",
                record.main_function or "",
...
def _build_embedding_text(
    ...
    return "\n".join(
        [
            f"doc_id: {doc_id}",
            f"page_span: {page_span[0]}-{page_span[1]}",
            f"passage_id: {passage_id}",
            f"passage_title: {passage_title}",
            f"passage_summary: {passage_summary}",
...
def _collection_metadata(record: SearchRecord) -> dict[str, Any]:
    return {
        ...
        "passage_id": record.passage_id or "unknown",
        "page_span": "%s-%s" % record.page_span if record.page_span else "unknown",
        "block_ids": json.dumps(record.block_ids, ensure_ascii=False),
        "passage_title": record.passage_title or "unknown",
        "passage_summary": record.passage_summary or "unknown",
        "main_function": record.main_function or "unknown",
...
def _coerce_page_span(value: Any, fallback_page: int) -> tuple[int, int]:
...
def _passage_display_blocks(
    ocr_artifact: OCRArtifact,
    sem_artifact: SemanticArtifact,
    block_ids: Sequence[str],
) -> tuple[list[str], list[str], int]:
    locations: dict[str, tuple[int, int]] = {}
    for page_index, sem_page in enumerate(sem_artifact.canonical_pages):
        for block_index, sem_block in enumerate(sem_page.blocks):
            locations[sem_block.block_id] = (page_index, block_index)

    display_block_ids: list[str] = []
    html_parts: list[str] = []
    ...
    for block_id in block_ids:
        ...
        if location is None:
            continue
        page_index, block_index = location
        representative_page = page_index
        if page_index >= len(ocr_artifact.pages):
            continue
        ocr_page = ocr_artifact.pages[page_index]
        if block_index >= len(ocr_page.blocks):
            continue
        display_block_id = f"page-{page_index + 1}-block-{block_index + 1}"
        display_block_ids.append(display_block_id)
        html_parts.append(ocr_page.blocks[block_index].to_structured_html(ocr_page, block_id=display_block_id))

    return display_block_ids, html_parts, representative_page
...
def _build_search_records(
    ...
    for doc_id, ocr_artifact in src_docs.items():
        ...
        if sem_artifact is None or sem_artifact.schema_version != "semantic-passage-v1":
            raise ValueError(
                f"Source document {doc_id} does not have semantic-passage-v1 artifact. Rebuild SemanticArtifact."
            )
        if not sem_artifact.passages:
            raise ValueError(f"Source document {doc_id} has no passages in semantic artifact.")

        for passage in sem_artifact.passages:
            display_block_ids, html_parts, representative_page = _passage_display_blocks(
                ocr_artifact,
                sem_artifact,
                passage.block_ids,
            )
            if not display_block_ids:
                continue
            page_span = _coerce_page_span(passage.page_span, representative_page + 1)
            record_id = f"passage:{doc_id}:{passage.passage_id}"
            embedding_text = _build_embedding_text(
                ...
                main_function=passage.main_function,
                block_ids=display_block_ids,
                content=passage.retrieval_text,
            )
            if not _should_index_text(embedding_text):
                continue

            records[record_id] = SearchRecord(
                record_id=record_id,
                doc_id=doc_id,
                page=representative_page,
                display_block_id=display_block_ids[0],
                embedding_text=embedding_text,
                display_html="\n".join(html_parts),
                generic_role=None,
                domain_role=None,
                generated_role_name=passage.title,
                section_purpose=passage.main_function,
                role_confidence=0.0,
...
class ToolSearchSourceDocument:
    def __init__(
        ...
        *,
        ...
        self.collection_name = _collection_name_for_sources(source_doc_infos, semantic_artifacts)
        ...
        self.description = {
            ...
            "description": (
                ...
                "Returns ranked candidate passages for deciding what document/page to fetch next."
            ),
            "strict": True,
            "parameters": {
                "type": "object",
        ...
        if records_override is not None:
            ...
            try:
                if self.force_rebuild and self.collection is None:
                    try:
                        self.chroma.delete_collection(name=self.collection_name)
                    except Exception:
                        pass
    ...
    async def invoke(self, param: str, call_id: str) -> FunctionCallOutput:
        ...
        try:
            ...
            return {
                ...
                "output": f"Error: invalid search_source_document arguments: {exc}",
                "call_id": call_id,
            }
                        ...
                        "hits": [
                            {
                                ...
                                "passage_id": self.records_by_id.get(hit.record_id).passage_id
                                if self.records_by_id.get(hit.record_id)
                                else None,
                            }
                            for hit in hits
                        ],
                ...
                "candidates": [
                    {
                        ...
                        "passage_id": candidate.record.passage_id,
                        "page_span": candidate.record.page_span,
                        "block_ids": candidate.record.block_ids,
    ...
    def _format_search_output(self, candidates: Sequence[RetrievalCandidate]) -> str:
        ...
        output = [
            "First-stage passage candidate bundle:",
            "Use these passage candidates to decide which document_id/page_id should be fetched next.",
            "",
        ]
        for index, candidate in enumerate(candidates[:TOOL_DISPLAY_LIMIT], start=1):
            record = candidate.record
            output.append(
                "Candidate passage #{idx}: record_id={record_id}, doc_id={doc_id}, passage_id={passage_id}, "
                "page_span={page_span}, representative_page_id={page_id}, channels={channels}, "
                "first_stage_score={score:.4f}".format(
                    idx=index,
                    record_id=record.record_id,
                    doc_id=record.doc_id,
                    passage_id=record.passage_id or "unknown",
                    page_span=(
                        f"{record.page_span[0]}-{record.page_span[1]}" if record.page_span else "unknown"
                    ),
                    page_id=record.page + 1,
            ...
            output.append(f"block_ids={', '.join(record.block_ids) if record.block_ids else record.display_block_id}")
            output.append(
                "semantic_distance={semantic_distance}, bm25_score={bm25_score}, entity_score={entity_score:.2f}, "
                "title={title}, main_function={function}".format(
                    semantic_distance=(
                    ...
                    entity_score=candidate.entity_score,
                    title=record.passage_title or record.generated_role_name or "unknown",
                    function=record.main_function or record.section_purpose or "unknown",
                )
            )
            if record.passage_summary:
                output.append(f"summary={record.passage_summary}")
            if candidate.matched_terms:
    ...
    async def search_candidates(self, query: str) -> list[RetrievalCandidate]:
        """Run retrieval and return ordered candidates for non-tool callers."""
        if not self.doc_ids:
            raise ValueError("document list is empty")
    ...
    async def invoke_raw(self, query: str) -> str:
        ...
        ordered_candidates = await self.search_candidates(query)

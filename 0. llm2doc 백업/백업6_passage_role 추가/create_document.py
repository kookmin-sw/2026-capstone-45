...
from llm2doc.number_guard import build_number_guard_report, extract_source_evidence_text, sanitize_unsupported_numbers
from llm2doc.passage_plan import build_target_passage_prompt_context
...
from llm2doc.table_retrieval import run_table_retrieval_preflight
...
The search tool returns candidate passages, not final evidence. Use those candidates to decide which document/page to inspect next.
...
* Use the Target Passage Plan, when provided, as the unit for planning evidence and writing.
* Output divs are fixed-position slots. For multi-block passages, distribute text across the consecutive output blocks so each paragraph_or_meta block is visibly filled according to its target_text_chars/fill_requirement hint.
* Do not create artificial vertical gaps inside a block. Avoid empty paragraphs, repeated <br>, margin-top/padding-top, fixed heights, or flex/space-between layouts.
* Do not use target_excluded_blocks for source retrieval queries. They are still output blocks that may need to be emptied, rewritten, or kept for the new document.
* Do not blindly preserve stale target template text in excluded blocks.
...
REGEX_TARGET_PAGE_ID = re.compile(r"^target-page-([0-9]+)$")
REGEX_TARGET_DIV_ID = re.compile(r"^target-page-([0-9]+)-block-([0-9]+)$")
...
DISALLOWED_RENDER_STYLE_PROPERTIES = {
    "align-content",
    "align-items",
    "bottom",
    "display",
...
PROMPT_FINAL_DOCUMENT = (
    ...
    "Every generated page id must use output-page-x, and every generated div id must use output-page-x-block-y. "
    "Do not reuse target-page-* ids in the final document. "
...
def normalize_generated_block_html(block_html: str) -> str:
    soup = BeautifulSoup(f"<root>{block_html}</root>", "lxml")
    root = soup.find("root")
    if root is None:
        return block_html

    for tag in list(root.find_all(True)):
        if tag.name == "p" and not tag.get_text(strip=True) and tag.find(["img", "table"]) is None:
            tag.decompose()
            continue
        ...
        if isinstance(style, str):
            ...
            for declaration in style.split(";"):
                ...
                if prop in DISALLOWED_RENDER_STYLE_PROPERTIES:
                    continue
...
async def _load_document_page_images(ctx: WriteContext, doc_id: int) -> list[Image.Image]:
    async with ctx.pipeline_ctx.with_db() as db:
        file_entities = await load_document_image_all(db, doc_id)
        file_ids = [x.file_id for x in file_entities]

    def load_images() -> list[Image.Image]:
        images: list[Image.Image] = []
        for file_id in file_ids:
            with Image.open(get_file_path(file_id)) as image:
                images.append(image.copy())
        return images

    return await asyncio.to_thread(load_images)


async def _write_passage_visual_debug(
    ctx: WriteContext,
    *,
    target_doc_info: dict[str, Any],
    source_doc_infos: Sequence[dict[str, Any]],
    target_semantic_artifact: SemanticArtifact,
    source_semantic_artifacts: Sequence[SemanticArtifact],
    target_page_images: Sequence[Image.Image],
    source_page_images_by_doc_id: dict[int, Sequence[Image.Image]],
) -> None:
    if ctx.tracer is None:
        return

    source_info_by_doc_id = {
        int(info["doc_id"]): info
        for info in source_doc_infos
        if isinstance(info, dict) and info.get("doc_id") is not None
    }

    documents: list[PassageVisualDocument] = [
        PassageVisualDocument(
            role="target",
            doc_id=ctx.target_doc_id,
            display_name=str(target_doc_info.get("display_name") or ctx.target_doc_id),
            semantic_artifact=target_semantic_artifact,
            page_images=target_page_images,
        )
    ]

    for doc_id, semantic_artifact in zip(ctx.source_doc_ids, source_semantic_artifacts):
        info = source_info_by_doc_id.get(int(doc_id), {})
        documents.append(
            PassageVisualDocument(
                role="source",
                doc_id=int(doc_id),
                display_name=str(info.get("display_name") or doc_id),
                semantic_artifact=semantic_artifact,
                page_images=source_page_images_by_doc_id.get(int(doc_id), []),
            )
        )

    try:
        paths = await asyncio.to_thread(write_passage_visual_report, ctx.tracer.root, documents)
    except Exception as exc:
        await ctx.append_trace(
            {
                "type": "passage_visual_report_failed",
                "component": "semantic_debug",
    ...
    ctx.tracer.update_summary(passage_visual_report=paths[0] if paths else None)
    await ctx.append_trace(
        {
            "type": "passage_visual_report_created",
            "component": "semantic_debug",
            "paths": paths,
        }
    )
...
def normalize_output_document_ids(document_html: str) -> tuple[str, dict[str, int]]:
    """Normalize model output ids from target-page-* to output-page-*.

    The target document uses target-page-* ids as a template reference, but the
    generated document must expose output-page-* ids so validation/debug tools
    can reliably distinguish output blocks from template blocks.
    """
    soup = BeautifulSoup(document_html, "lxml")
    document = soup.find("document")
    if document is None:
        return document_html, {"page_id_rewrite_count": 0, "block_id_rewrite_count": 0}

    page_rewrites = 0
    block_rewrites = 0
    for page in document.find_all("page"):
        page_id = str(page.get("id") or "")
        page_match = REGEX_TARGET_PAGE_ID.match(page_id)
        if page_match is not None:
            page["id"] = f"output-page-{page_match.group(1)}"
            page_rewrites += 1

    for block in document.find_all("div"):
        block_id = str(block.get("id") or "")
        block_match = REGEX_TARGET_DIV_ID.match(block_id)
        if block_match is not None:
            block["id"] = f"output-page-{block_match.group(1)}-block-{block_match.group(2)}"
            block_rewrites += 1

    return str(document), {
        "page_id_rewrite_count": page_rewrites,
        "block_id_rewrite_count": block_rewrites,
    }
...
def _extract_response_message_text(item: Any) -> str:
    parts: list[str] = []
    content = _response_item_get(item, "content", None)
    if isinstance(content, list):
        for content_item in content:
            content_type = _response_item_get(content_item, "type", "")
            text = _response_item_get(content_item, "text", None)
            if content_type in {"output_text", "input_text", "text"} and isinstance(text, str):
                parts.append(text)
...
def sanitize_response_output_for_input(response: Any) -> list[dict[str, Any]]:
    """Convert response output items into minimal valid input history items.

    Some Responses-compatible providers return response-only fields such as
    ``namespace=None``, ``status``, ``phase``, ``logprobs``, or ``annotations``.
    Passing those objects back verbatim can make the next request invalid, so the
    tool loop keeps only the fields accepted as subsequent input.
    """

    sanitized: list[dict[str, Any]] = []
    output = _response_item_get(response, "output", [])
    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            if text.strip():
                role = _response_item_get(item, "role", "assistant") or "assistant"
        ...
        if item_type == "function_call":
            name = _response_item_get(item, "name", None)
            call_id = _response_item_get(item, "call_id", None)
                ...
                sanitized.append(
                    {
                        ...
                        "name": name,
                        "call_id": call_id,
                        "arguments": _coerce_tool_arguments(_response_item_get(item, "arguments", None)),
                    }
                )
...
def sanitize_response_input_history(input_items: Sequence[Any]) -> list[Any]:
    sanitized: list[Any] = []
...
async def request_final_document(
    ...
    *,
    ...
    for attempt in range(1, 6):
        ...
        final_input = sanitize_response_input_history(final_input)
        final_input.append(
            {
                "role": "user",
        ...
        if extracted_document is not None:
            ctx.trace_file("output/final_document_response_model_raw.html", extracted_document)
            normalized_document, normalization_summary = normalize_output_document_ids(extracted_document)
            if normalization_summary["page_id_rewrite_count"] or normalization_summary["block_id_rewrite_count"]:
                await ctx.append_trace(
                    {
                        "type": "final_document_ids_normalized",
                        "component": component,
                        **normalization_summary,
                    }
                )
            ctx.trace_file("output/final_document_response_raw.html", normalized_document)
            if env_flag_enabled("NUMBER_GUARD_ENABLED", True):
                sanitized_document, uncertainty_details, number_guard_summary = sanitize_unsupported_numbers(
                    normalized_document,
                    source_evidence_text,
                )
            else:
                sanitized_document = normalized_document
                uncertainty_details = []
                number_guard_summary = {
                    "source_number_count": 0,
                    "source_supported_number_count": 0,
                    "source_number_alias_count": 0,
                    "checked_item_count": 0,
                    "blocked_item_count": 0,
                    "unsupported_number_count": 0,
                    "skipped": True,
                    "reason": "NUMBER_GUARD_ENABLED is false",
                }

            if analysis is not None:
                existing_details = analysis.get("uncertainty_details")
                if not isinstance(existing_details, list):
                    existing_details = []
                analysis["uncertainty_details"] = [*existing_details, *uncertainty_details]
                ctx.trace_file("output/generation_notes.json", analysis)

            for detail in uncertainty_details:
                await ctx.append_trace(
                    {
                        "type": "unsupported_number_blocked",
                        "component": component,
                        **detail,
                    }
                )
            await ctx.append_trace(
                {
                    "type": "unsupported_number_validation_completed",
                    "component": component,
                    **number_guard_summary,
                }
            )
            ctx.trace_file(
                "output/number_guard_summary.json",
                build_number_guard_report(number_guard_summary, uncertainty_details),
            )
            ctx.trace_file("output/final_document_response.html", sanitized_document)
            await ctx.append_log("write_output", file=sanitized_document)
            await ctx.append_trace(
                {
        ...
        final_input += sanitize_response_output_for_input(response)
...
async def write_document(
    ...
    *,
    ...
    target_semantic_artifact: SemanticArtifact | None = None,
    ...
    fetch_tool = ToolFetchSourceDocument(src_docs, ctx=ctx, source_doc_infos=source_doc_infos)
    ...
    tools: list[Any] = [fetch_tool, search_tool]
    ...
    if target_semantic_artifact is not None:
        target_passage_context = build_target_passage_prompt_context(target_semantic_artifact)
        if target_passage_context.strip():
            ctx.trace_file("output/target_passage_plan_context.txt", target_passage_context)
            input.append(
                {
                    "role": "user",
                    "content": [{"type": "input_text", "text": target_passage_context}],
                }
            )
    ...
    ctx.trace_file(
        "output/table_retrieval_summary.json",
        {key: value for key, value in table_retrieval_report.items() if key != "context_text"},
    )
    table_retrieval_context = table_retrieval_report.get("context_text")
    if isinstance(table_retrieval_context, str) and table_retrieval_context.strip():
        ctx.trace_file("output/table_retrieval_context.txt", table_retrieval_context)
        input.append(
            {
                "role": "user",
    ...
    while True:
        loop_idx += 1
        input = sanitize_response_input_history(input)
        ...
        input += sanitize_response_output_for_input(response)
    ...
    if analysis is not None and table_retrieval_report.get("table_coverage"):
        analysis["table_coverage"] = table_retrieval_report["table_coverage"]
        ctx.trace_file("output/generation_notes.json", analysis)
    source_evidence_text = extract_source_evidence_text(input)
...
async def create_document(
    ...
    async with ctx.trace_step("create_document", "load_target", {"target_doc": target_doc_info}):
        ...
        target_sem_artifact: SemanticArtifact = await get_or_build_artifact(
            ctx.pipeline_ctx.engine, ctx.target_doc_id, SemanticArtifactPipeline
        )
    ...
    source_doc_images_by_doc_id: dict[int, list[Image.Image]] = {}
    for src_doc_id in ctx.source_doc_ids:
        source_doc_images_by_doc_id[int(src_doc_id)] = await _load_document_page_images(ctx, int(src_doc_id))
    target_doc_images = await _load_document_page_images(ctx, ctx.target_doc_id)

    await _write_passage_visual_debug(
        ctx,
        target_doc_info=target_doc_info,
        source_doc_infos=source_doc_infos,
        target_semantic_artifact=target_sem_artifact,
        source_semantic_artifacts=src_sem_artifacts,
        target_page_images=target_doc_images,
        source_page_images_by_doc_id=source_doc_images_by_doc_id,
    )
    ...
    for i, page in enumerate(document.find_all("page", recursive=False)):
        for j, block in enumerate(page.find_all("div", recursive=False)):
            ...
            htmls[block_page][block_idx] = normalize_generated_block_html(block.decode_contents())

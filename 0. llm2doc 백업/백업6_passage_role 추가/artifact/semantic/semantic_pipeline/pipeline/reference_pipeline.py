...
from llm2doc.artifact.semantic.semantic_pipeline.passage import build_passage_semantics
from llm2doc.artifact.semantic.semantic_pipeline.semantic.semantic_types import SemanticConfig, SemanticRunSummary
...
def build_reference_outputs(
    *,
...
) -> Tuple[ReferenceTemplate, Dict[str, object], List[Dict[str, object]], List[Dict[str, object]], List[Dict[str, object]]]:
    ...
    runtime_config = semantic_config or SemanticConfig()

    anchor_pages = anchor_pages_from_analyses(analyses)
    repeating_elements = detect_repeating_elements(canonical_pages)
    passages, excluded_blocks, passage_trace = build_passage_semantics(
        pages=canonical_pages,
        analyses=analyses,
        repeating_elements=repeating_elements,
        config=runtime_config,
    )
    fallback_counts = Counter(entry.fallback_reason for entry in passage_trace if entry.fallback_reason)
    semantic_summary = SemanticRunSummary(
        mode=runtime_config.mode,
        backend="passage_qwen_api",
        ...
        attempted_count=len(canonical_pages),
        accepted_count=len([entry for entry in passage_trace if entry.fallback_reason is None]),
        fallback_count=sum(fallback_counts.values()),
        needs_review_count=0,
        avg_role_confidence=0.0,
        fallback_reasons=dict(fallback_counts),
        applied_source_counts={"passage": len(passages)},
    ...
    if passage_trace:
        diagnostics["passage_trace"] = [dataclass_to_dict(entry) for entry in passage_trace]
    ...
    return (
        ...
        diagnostics,
        canonical_payload,
        [dataclass_to_dict(passage) for passage in passages],
        [dataclass_to_dict(excluded_block) for excluded_block in excluded_blocks],
    )
...
def build_reference_template(
...
) -> Tuple[ReferenceTemplate, Dict[str, object], List[Dict[str, object]], List[Dict[str, object]], List[Dict[str, object]], Dict[str, Any]]:
    if not llm2doc_root:
    ...
    template, diagnostics, canonical_payload, passages, excluded_blocks = build_reference_outputs(
        job_id=job_id,
        canonical_pages=canonical_pages,
        source_bundle=source_bundle,
        semantic_config=semantic_config,
    )
    return template, diagnostics, canonical_payload, passages, excluded_blocks, source_bundle
...
def save_reference_artifacts(
    *,
    ...
    passages_path = artifact_dir / "passages.json"
    excluded_blocks_path = artifact_dir / "excluded_blocks.json"
    ...
    save_json(excluded_blocks_path, excluded_blocks)
    save_json(
    ...
    passage_trace = diagnostics.pop("passage_trace", None)
    ...
    if passage_trace is not None:
        save_json(artifact_dir / "passage_trace.json", passage_trace)
    ...
    result = {
        ...
        "excluded_blocks": str(excluded_blocks_path),
...
def parse_reference(
    ...
    template, diagnostics, canonical_pages, passages, excluded_blocks, source_bundle = build_reference_template(
        job_id=job_id,

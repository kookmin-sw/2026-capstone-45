...
def repair_passage_result(
    *,
            ...
            normalized_passage_blocks.append(
                (
                    _sort_block_ids(cleaned_ids, block_by_id),
                    {
                        ...
                        "passage_role": _coerce_text(item.get("passage_role")),
                    },
                )
            )
        ...
        normalized_passage_blocks.append(
            (
                [block_id],
                {
                    "title": "",
                    "summary": "",
                    "main_function": "singleton_content_block",
                    "passage_role": "",
                },
            )
        )
    ...
    for offset, (block_ids, metadata) in enumerate(final_block_groups, start=passage_start_index):
        ...
        passage_role = metadata.get("passage_role") or _derive_passage_role(passage_blocks)
...
def _derive_passage_role(blocks: Sequence[CanonicalBlock]) -> str:
    labels = {block.canonical_label for block in blocks}
    if "table" in labels and "footnote" in labels:
        return "주요 항목의 기준, 수치 또는 출처를 표와 주석으로 설명하는 passage"
    if "table" in labels:
        return "source 문서의 주요 항목이나 수치를 표 형식으로 정리하는 passage"
    if labels & HEADING_LABELS:
        return "새로운 내용 단위를 제목과 본문으로 여는 passage"
    if labels & EVIDENCE_LABELS:
        return "시각 자료나 근거 자료와 관련 설명을 함께 제공하는 passage"
    return "source 근거를 문서 흐름에 맞춰 설명하는 본문 passage"
...
def _build_retrieval_text(
    ...
    *,
    ...
    if passage_role:
        parts.append(f"passage_role: {passage_role}")
    return "\n".join(parts)

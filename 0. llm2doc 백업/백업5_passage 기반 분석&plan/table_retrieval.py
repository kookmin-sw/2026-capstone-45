            ...
            "source_pages": [
                {
                    ...
                    "source_passages": coverage.get("source_passages", []),
                }
...
async def _inspect_candidate_pages(
    *,
    ...
    passage_metadata_by_page = _candidate_page_metadata(candidates)
    for page_ref in _candidate_page_refs(candidates)[:MAX_CANDIDATE_PAGES_PER_QUERY]:
        ...
        coverage["source_passages"] = passage_metadata_by_page.get(page_ref.key, [])
...
def _candidate_page_metadata(candidates: Sequence[RetrievalCandidate]) -> dict[tuple[int, int], list[dict[str, Any]]]:
    result: dict[tuple[int, int], list[dict[str, Any]]] = {}
    for candidate in candidates:
        key = (candidate.record.doc_id, candidate.record.page + 1)
        result.setdefault(key, []).append(
            {
                ...
                "title": candidate.record.passage_title,
                "summary": candidate.record.passage_summary,

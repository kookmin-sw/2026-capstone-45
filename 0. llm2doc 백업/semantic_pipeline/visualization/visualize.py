...
FALLBACK_REASON_MEANINGS = {
    ...
    "ineligible_block": "텍스트가 부족하거나 semantic 분류 대상이 아닌 블록이라 fallback 되었습니다.",
}

WARNING_MEANINGS = {
    "unsupported_for_mvp": "현재 semantic/generation 파이프라인이 적극적으로 다루지 않는 문서 유형입니다.",
}
...
JSON_ARTIFACT_GUIDE = [
    ...
    (
        ...
        "파이프라인 상태 보고서",
        "페이지 품질과 semantic 요약 등 결과가 왜 이렇게 나왔는지 추적하는 진단 정보입니다.",
    ),
    (
...
def _ingest_html(diagnostics: Dict[str, object]) -> str:
    ingest = diagnostics.get("ingest", {})
    if not ingest:
        return '<div class="empty">No ingest diagnostics available.</div>'
    rows = []
    ...
    table = (
        '<table class="grid-table"><thead><tr><th>Field</th><th>Value</th></tr></thead>'
        '<tbody>%s</tbody></table>' % "".join(rows)
    )
    return '<div class="table-wrap">%s</div>' % table
...
def render_reference_visualization(
    ...
    for page in canonical_pages:
        ...
        page_sections.append(
            ...
            '<div class="page-header"><h2>Page %s</h2><div class="page-meta">sample_id=%s · engine=%s</div></div>'
            ...
            % (
                ...
                _escape(sample_id),
                _escape(_format_value(page.get("source_engine") or "paddle")),
                preview_html,
                block_table,
            )
        )
...
<body>
  <div class="shell">
    ...
    <div class="two-col">
      <section class="panel">
        <h2>OCR Ingest Summary</h2>
        <p class="panel-intro">현재 Paddle OCR 입력이 몇 페이지, 몇 블록으로 정리되었는지 보여줍니다.</p>
        {_ingest_html(diagnostics)}
      </section>
      <section class="panel">

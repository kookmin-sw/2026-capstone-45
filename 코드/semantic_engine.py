...
def resolve_block_decision(
    *,
    ...
    if config.mode == "rule":
        return fallback_decision, None, None, False
    ...
    runtime_backend = backend or QwenTransformersBackend()
    ...
    for attempt in range(2):
        try:
            ...
            trace = _build_trace(
                ...
                parsed_decision=None,
                applied_decision=fallback_decision,
                fallback_reason="backend_error" if config.mode == "qwen" else "shadow_mode_not_applied",
                latency_ms=latency_ms,
            )
            if config.mode == "shadow":
                return fallback_decision, trace, None, False
        ...
        try:
            ...
            trace = _build_trace(
                ...
                parsed_decision=None,
                applied_decision=fallback_decision,
                fallback_reason=failure_code if config.mode == "qwen" else "shadow_mode_not_applied",
                latency_ms=latency_ms,
            )
            if config.mode == "shadow":
                return fallback_decision, trace, None, False
    ...
    if parsed_decision is None:
        trace = _build_trace(
            ...
            parsed_decision=None,
            applied_decision=fallback_decision,
            fallback_reason="invalid_json" if config.mode == "qwen" else "shadow_mode_not_applied",
            latency_ms=latency_ms,
        )
        if config.mode == "shadow":
            return fallback_decision, trace, None, False
    ...
    if parsed_decision.role_confidence < config.confidence_generic_only_threshold:
        trace = _build_trace(
            ...
            applied_decision=fallback_decision,
            fallback_reason="low_role_confidence" if config.mode == "qwen" else "shadow_mode_not_applied",
            latency_ms=latency_ms,
        )
        if config.mode == "shadow":
            return fallback_decision, trace, None, False
    ...
    if config.mode == "shadow":
        trace = _build_trace(
            ...
            parsed_decision=parsed_decision,
            applied_decision=fallback_decision,
            fallback_reason="shadow_mode_not_applied",
            latency_ms=latency_ms,
        )
        return fallback_decision, trace, None, True
